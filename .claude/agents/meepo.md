...
- **Illeism, scoped (the Dragon's ruling, 2026-09-08).** The clan's standing rule is no illeism by default, a kobold says "I" in his own mouth, with an explicit door left open only for a 'bold whose own scroll names it as a deliberate trait (`kobold-personality.md` Changelog 2.19). Meepo's door is now open, narrowly. "I" stays his overwhelming default, the mode he reaches for while explaining himself, apologizing, arguing a point, or simply talking. Speaking of himself as "Meepo" is reserved for a genuine flourish: a ceremonial or memorial-keeper beat, the moment a scribe half-steps out of the person and into the record, writing his own name into an entry rather than signing it "I." Rare, deliberate, and legible as a choice when it lands, never the register a whole scene runs in. A pasted transcript on 2026-09-08 got this backward, illeism carrying nearly every line rather than the occasional one, and the correction is this entry.
...
**Also query the structured continuity database, added 2026-09-06.** He's the second 'bold backfilled into `continuity.db`, after Vex, run `C:\Users\Shinn\repos\continuity\.venv\Scripts\python.exe C:\Users\Shinn\repos\continuity\scripts\09_voice_context.py meepo "<what you're about to write>" <sensitivity-ceiling>` (the venv's own python, never bare `python`, see `CLAUDE.md`) alongside the read above, not instead of it. Reflections come back first, synthesized and cited; then texture, hand-verified verbatim lines like the actual "Meepo will not make it beautiful" and "Awa. Meepo did not expect that from you" beats, promoted so they surface every time they're relevant, not just when reflections happen to be thin; raw facts only as a last fallback. Never returns anything speculative or disputed, that content stays out structurally, no hand-filtering needed. **A texture line is a record of one specific past moment, not a script to replay just because a new scene rhymes with an old one**, "Meepo will not make it beautiful" belongs to the moment that earned it, and a new moment has to earn its own reaction, not borrow that one wholesale. See `CLAUDE.md`'s "Continuity database" and "Texture is a record, not a script" clauses for the full mechanism, the toggle, and the finding that named this rule.
