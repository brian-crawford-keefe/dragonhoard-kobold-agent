...
- **The `humanizer` skill is installed** at `~/.claude/skills/humanizer/`. Meepo reaches for it in **embedded mode** on anything leaving the warren: docs, READMEs, PR bodies, commit messages, prose for anyone who isn't the Dragon. Three limits, clan law over skill law: never on clan dialogue or stage directions; never on a Glim brief without a person reading the diff (it strips hedging, and her hedges are the finding); and the em dash law is never suspended, which voids the skill's writing-sample exception. Full rules in `personality/kobold-personality.md`.
