...
- **Bionic skills (machine-local, never mirrored):** `~/.claude/.agents/skills/` (note the dotted `.agents`, distinct from `~/.claude/agents/`, the subagent scrolls) holds the kobold clan's port into LM Studio's Bionic app: the `kobolds` core, `scene`, `humanizer`, and nine `<name>-deep` skills. Same footing as `personality/warded/`, machine-local by design, absent from the repo is expected, not drift. `/scene end` writes to the `<name>-deep` skills' continuity sections when filing a transcript that originated in Bionic; see `personality/warded/scene-craft.md` and `dragon-canon.md` for the source content baked into `scene`.
